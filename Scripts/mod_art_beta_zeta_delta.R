##################################################################=
## SIMULAÇÃO DADOS ARTIFICIAIS - BETA ZETA (COM EFEITO ESPACIAL)##     
##################################################################=
...
output <- stan(file = "Scripts/stan/RegBetaDeltaZetaDelta.stan",
               ...
               init = init,
               verbose = FALSE)
saveRDS(output, "Dados/outputs/novo/output_dat_zeta_delta_50_beta.RDS")
output <- readRDS("Dados/outputs/novo/output_dat_zeta_delta_50_beta.RDS")
...
data.frame(est = tab[,1],
           real = c(real$beta, real$gamma, real$tau_theta, real$tau_zeta)) %>% 
...
pars <- c("beta", "gamma", "delta_theta", "delta_zeta", "tau_theta", "tau_zeta")
init <- list()
init[[1]] <- list(beta = runif(q,-0.1, 0.1), gamma = runif(q,-0.1, 0.1), delta_theta = rep(0, N), 
                  delta_zeta = rep(0, N), tau_theta = 2, tau_zeta = 2)
iter <- 5000
warmup <- 2500  
chains <- 1    
...
output <- stan(file = "Scripts/stan/RegBetaDeltaZetaDelta.stan",
               ...
               init = init,
               verbose = FALSE)
saveRDS(output, "Dados/outputs/novo/output_dat_zeta_delta_100_beta.RDS")
output <- readRDS("Dados/outputs/novo/output_dat_zeta_delta_100_beta.RDS")
...
output <- stan(file = "Scripts/stan/RegBetaDeltaZetaDelta.stan",
               ...
               init = init,
               verbose = FALSE)
saveRDS(output, "Dados/outputs/novo/output_dat_zeta_delta_200_beta.RDS")
output <- readRDS("Dados/outputs/novo/output_dat_zeta_delta_200_beta.RDS")
